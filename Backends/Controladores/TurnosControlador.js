...
exports.buscarTurnos = async (req, res) => {
    try {
        const { deporte, fecha, duracion } = req.query; // Recibimos filtros por URL
        ...
        const turnos = await Turno.findAll({
            ...
            include: [{
                ...
                include: [{
                    model: Deporte,
                    where: { nombre: deporte } // Filtramos por el nombre del deporte
                }]
            }]
        });
        res.json(turnos);
    } catch (error) {
        res.status(500).json({ msg: 'Error al buscar turnos.' });
    }
};